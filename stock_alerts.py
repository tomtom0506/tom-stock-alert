...
def update_portfolio_simulation(store):
    ...
    sim = store.setdefault("portfolio_sim", {
        "start_value": 100000,
        "currency": "ILS",
        "value": 100000.0,
    ...
    graded_top10 = [
        ...
        if e.get("top10") and e.get("graded") and e.get("predicted") == "up"
    ]
    if not graded_top10:
        return
...
def run_predictions(store):
    ...
    for entry in today_entries:
        if entry["ticker"] in chart_eligible and entry["ticker"] in chart_data_by_ticker:
            entry["chart"] = chart_data_by_ticker[entry["ticker"]]
