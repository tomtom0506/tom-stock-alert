...
def run_predictions(store):
    ...
    for symbol in technical:
        ...
        if is_strong:
            # only fetch news for the handful of "strong" picks (another slow
            # network call) - this is a link to something current to go read,
            # not a scored sentiment signal (see get_latest_news docstring)
            try:
    ...
    # a short, curated "top picks" list (by conviction) so the dashboard has
    # something to lead with instead of every single stock that crossed the
    # (fairly loose) alert threshold - full history is still kept for every
    # ticker, for grading/accuracy tracking.
    today_entries = [e for e in store["history"] if e["date"] == today]
    top_picks = sorted(today_entries, key=lambda e: abs(e["score"]), reverse=True)[:15]
    store["top_picks"] = {"date": today, "tickers": [e["ticker"] for e in top_picks]}

    if us_strong:
        send_telegram_message_chunked(
            "🔮 חיזוי ליום המסחר הבא - שוק ארה\"ב", us_strong, parse_mode="Markdown",
        )
    if il_strong:
        send_telegram_message_chunked(
            "🔮 חיזוי ליום המסחר הבא - בורסת ת\"א", il_strong, parse_mode="Markdown",
        )
