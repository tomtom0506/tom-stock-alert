...
def grade_pending_predictions(store):
    ...
    today = date.today().isoformat()
    pending = [e for e in store["history"] if not e.get("graded") and e.get("date") != today]
    if not pending:
        return False

    tickers = sorted({e["ticker"] for e in pending})
    print(f"Grading {len(pending)} pending predictions across {len(tickers)} tickers...")
...
def update_portfolio_simulation(store, flag_key="top10", sim_key="portfolio_sim"):
    ...
    new_dates = sorted(d for d in by_date if last is None or d > last)
...
def run_predictions(store):
    ...
    chart_eligible = curated_tickers | starred_tickers
...
def is_market_trading_day():
    """Whether the US market actually traded today, checked against real
    data (SPY's latest bar) rather than a hardcoded weekday rule - a
    weekday check alone misses market holidays, and generating "new"
    predictions or grading old ones against a stale non-trading closing
    price silently corrupts the stats (a 0%-change day gets counted as a
    correct "up" prediction, since 0 counts as >= 0)."""
    try:
        hist = yf.Ticker("SPY").history(period="5d")
        if hist.empty:
            return True  # fail open - a data hiccup shouldn't halt the whole system
        last_trading_date = hist.index[-1].date()
        return last_trading_date == date.today()
    except Exception as e:
        print(f"Could not verify trading day, proceeding anyway: {e}")
...
def main():
    ...
    if not is_market_trading_day():
        print("Market didn't trade today (weekend/holiday) - skipping grading and new predictions.")
